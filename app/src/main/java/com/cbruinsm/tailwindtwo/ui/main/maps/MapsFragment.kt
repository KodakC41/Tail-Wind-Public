...
import com.cbruinsm.tailwindtwo.ui.main.plane.ViewModels.trackerViewModel
...
class MapsFragment : Fragment(), SharedPreferences.OnSharedPreferenceChangeListener {
     ...
     * Flight Tracker
     */
    private lateinit var TRACK : trackerViewModel
    ...
    //Map Callback that plots for flight path : important checkpoints.
     private val flightPath = OnMapReadyCallback { googleMap ->
        ...
        TRACK.flightX.observe(viewLifecycleOwner) {
            Tracker(it,googleMap)
        }
    }
    private fun Router(it : ArrayList<fixes>, googleMap : GoogleMap) {
    ...
    private fun Tracker(it : ArrayList<positions>, googleMap : GoogleMap) {
        val routes = ArrayList<LatLng>()
            googleMap.clear()
            for(i in 0 until it.size) {
                routes.add(LatLng(it[i].latitude, it[i].longitude))
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?
        ...
        TRACK = ViewModelProvider(requireActivity()).get(trackerViewModel::class.java)
